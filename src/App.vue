<template>
  <div class="app-container">
    ...
    <header>
      ...
      <h1>{{ appTitle }}</h1>
      <p>黄金前3秒钩子 · 剧本大纲 · 人设张力 · 卡点反转 · 拍摄分镜金句</p>
    </header>
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="selector-group">
        <label class="selector-label">选择剧本策划类型</label>
        <div class="style-selector">
          <button 
            v-for="stype in scriptTypeOptions" 
            :key="stype.value"
            class="style-option"
            :class="{ active: activeScriptType === stype.value }"
            @click="activeScriptType = stype.value"
          >
            {{ stype.label }}
          </button>
        </div>
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        ...
        <div class="selector-group">
          <label class="selector-label">集数规格</label>
          <div class="style-selector">
            <button 
              v-for="len in episodeLengthOptions" 
              :key="len"
              class="style-option"
              :class="{ active: selectedEpisodeLength === len }"
              @click="selectedEpisodeLength = len"
            >
              {{ len }}
            </button>
          </div>
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          <label class="selector-label">输入微短剧故事核心梗概、人设亮点或爆点诉求</label>
          <div style="display: flex; gap: 0.5rem;">
            <button v-if="userInput" class="text-link-btn" @click="userInput = ''">清空输入</button>
            <button class="text-link-btn" @click="showScriptRulesModal = true">微短剧爆款爽点与留存率铁律指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
          placeholder="请简要描述您的微短剧故事构想或爽点需求...（例如：战神龙王隐藏身份入赘豪门，被岳母大娘肆意践踏，女儿被反派打伤后暴怒恢复身份，请生成黄金前3秒、主要角色人设、5集卡点大纲及分镜金句。）"
          style="min-height: 130px;"
        ></textarea>
        <div style="display: flex; justify-content: space-between; font-size: 0.75rem; color: var(--text-secondary);">
          <span>字符数: {{ userInput.length }} 字</span>
          <span>建议包含主角人设、核心矛盾冲突、爆点钩子及集数需求</span>
        </div>
      </div>
    ...
    <section v-if="result || loading" class="glass-card">
      <div class="result-header">
        <span class="result-title">微短剧剧本与爆点策划报告</span>
        <div class="button-actions">
          <button v-if="result" class="icon-btn" @click="copyText">
            {{ copied ? '已复制剧本全文' : '复制剧本策划' }}
          </button>
      ...
      <div v-else-if="result">
        ...
        <div v-if="aiScores" class="scores-container" style="margin-bottom: 1.5rem; padding: 1.25rem; background: rgba(0,0,0,0.25); border-radius: 12px; border: 1px solid rgba(255,255,255,0.06);">
          <div style="font-weight: 700; font-size: 0.95rem; margin-bottom: 1rem; color: #a5b4fc; display: flex; justify-content: space-between; align-items: center;">
            <span>AI 短剧戏剧张力与留存率评估看板</span>
            <span style="font-size: 0.8rem; font-weight: normal; color: var(--text-secondary);">综合爆款分: {{ getAverageScoreFromMap(aiScores) }} / 5.0</span>
          </div>
    ...
    <section v-if="showHistory" class="glass-card" style="margin-top: 1rem;">
      <div class="result-header">
        <span class="result-title">本地微短剧剧本历史记录</span>
      ...
      <div v-else class="history-grid" style="display: flex; flex-direction: column; gap: 0.75rem; max-height: 320px; overflow-y: auto;">
        <div v-for="item in historyList" :key="item.id" class="history-item" style="padding: 1rem; background: rgba(0,0,0,0.2); border-radius: 10px; border: 1px solid var(--card-border);">
          <div style="display: flex; justify-content: space-between; font-size: 0.8rem; color: var(--text-secondary); margin-bottom: 0.4rem;">
            <span>{{ item.timestamp }} · [{{ item.scriptType }} / {{ item.platform }}]</span>
            <span style="color: var(--primary-color);">爆款分: {{ getAverageScore(item) }}</span>
          </div>
          ...
          <div style="display: flex; gap: 0.5rem;">
            <button class="icon-btn" style="font-size: 0.75rem;" @click="applyHistory(item)">套用预设</button>
            <button class="icon-btn" style="font-size: 0.75rem;" @click="viewHistoryOutput(item)">查看剧本全文</button>
          </div>
        </div>
      </div>
    </section>
    ...
    <!-- 微短剧爆款爽点与留存率铁律指南 Modal -->
    <div v-if="showScriptRulesModal" class="modal-overlay" @click.self="showScriptRulesModal = false">
      <div class="modal-content" style="max-width: 480px;">
        <h3>微短剧爆款爽点与留存率铁律指南</h3>
        <p style="text-align: left; font-size: 0.825rem; margin-bottom: 1rem; color: var(--text-secondary);">
          提高短剧完播率与充值转化率的三大核心法则：
        </p>
        <div class="modal-scroll-area" style="text-align: left; font-size: 0.825rem;">
          <div v-for="(rule, idx) in scriptRules" :key="idx" style="margin-bottom: 0.75rem; padding: 0.5rem 0.75rem; background: rgba(255,255,255,0.03); border-radius: 8px; border: 1px solid rgba(255,255,255,0.05);">
        ...
        <button class="modal-btn" style="margin-top: 1rem;" @click="showScriptRulesModal = false">关闭</button>
      </div>
    </div>
    ...
    <div v-if="showShareGuide" class="modal-overlay" @click.self="showShareGuide = false">
      <div class="modal-content">
        <h3>分享微短剧剧本与爆点策划工具</h3>
        <p>扫码关注或将链接转发给短剧编剧、导演与制片团队，爆款卡点一键全拿。</p>
    ...
    <div v-if="showPrivacy" class="modal-overlay" @click.self="showPrivacy = false">
      <div class="modal-content">
        ...
        <div class="modal-text-content modal-scroll-area">
          <p>我们重视您的剧本创意与版权隐私。您在本工具中输入的微短剧故事梗概与核心卡点仅用于大模型实时生成，系统不会在云端公开您的创意剧本。</p>
    ...
    <div v-if="showTerms" class="modal-overlay" @click.self="showTerms = false">
      <div class="modal-content">
        ...
        <div class="modal-text-content modal-scroll-area">
          <p>欢迎使用网腾无限 AI 微短剧剧本与爆点策划专家。本工具生成的剧本大纲、分镜提示与台词金句供短剧创作与拍摄预演参考。</p>
          <p>正式开机拍摄前，请根据具体预算、演员阵容与平台审核要求进行精细化修润与二次创作。</p>
        </div>
...
// 配置参数
const appTitle = ref(appConfig.title || '网腾无限AI - 微短剧剧本与爆点策划专家');
...
const showScriptRulesModal = ref(false);
...
const scriptTypeOptions = [
  { label: '微短剧黄金前3秒与大纲策划', value: '微短剧黄金前3秒与大纲策划' },
  { label: '战神战王与爽文逆袭剧本', value: '战神战王与爽文逆袭剧本' },
  { label: '豪门恩怨与甜宠战术剧本', value: '豪门恩怨与甜宠战术剧本' },
  { label: '悬疑脑洞与科幻反转剧本', value: '悬疑脑洞与科幻反转剧本' }
];
const activeScriptType = ref(scriptTypeOptions[0].value);

// 目标平台与集数规格
const platformOptions = ['抖音短剧', '快手短剧', '微信小程序短剧', '海外Shorts'];
const selectedPlatform = ref('抖音短剧');

const episodeLengthOptions = ['1-3分钟短小精悍', '3-5分钟精致剧情', '80-100集长剧全案'];
const selectedEpisodeLength = ref('80-100集长剧全案');

// 评估指标列表
const metricsList = [
  { key: 'conflictIntensity', label: '戏剧冲突强度' },
  { key: 'goldenPacing', label: '黄金前3秒留存率' },
  { key: 'climaxReversal', label: '高潮反转爆点值' },
  { key: 'characterTension', label: '角色人设张力' },
  { key: 'shootingFeasibility', label: '拍摄分镜可行度' }
];
...
// 短剧爽点铁律
const scriptRules = [
  { title: '黄金3秒卡点律', advice: '第一镜头必须爆破悬念、生死危机或极致打脸，抓紧观众注意力', avoid: '严禁平铺直叙做长篇背景铺垫' },
  { title: '每集留存钩子律', advice: '每集末尾（50-80秒处）必须停留在情绪最高潮或悬念卡点（Cliffhanger），引发下翻冲动', avoid: '切忌在平淡的对话或问题解决后自然结束' },
  { title: '极端人设对立律', advice: '正邪阵营反差要拉满，主角爽点痛快淋漓，反派嚣张跋扈至极，形成强情绪拉扯', avoid: '忌人设中立模棱两可或反派动机过于软弱' }
];

// 计算纯结果文本 (剔除打分标签 [DUANJU_SCORES])
const displayResultText = computed(() => {
  if (!result.value) return '';
  return result.value.replace(/\[DUANJU_SCORES\][\s\S]*?\[\/DUANJU_SCORES\]/g, '').trim();
});

// 解析打分标签
const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[DUANJU_SCORES\](.*?)\[\/DUANJU_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('duanju_history_records');
...
const saveHistory = () => {
  localStorage.setItem('duanju_history_records', JSON.stringify(historyList.value));
};
...
const addHistoryRecord = () => {
  const newItem: HistoryItem = {
    ...
    scriptType: activeScriptType.value,
    platform: selectedPlatform.value,
    episodeLength: selectedEpisodeLength.value,
...
const applyHistory = (item: HistoryItem) => {
  ...
  activeScriptType.value = item.scriptType;
  selectedPlatform.value = item.platform;
  if (item.episodeLength) selectedEpisodeLength.value = item.episodeLength;
...
const handleGenerate = async () => {
  ...
  try {
    const response = await fetch(apiEndpoint, {
      ...
      body: JSON.stringify({
        taskType: 'text',
        prompt: `任务指导: ${promptTopic.value}\n【剧本策划类型】: ${activeScriptType.value}\n【目标平台】: ${selectedPlatform.value}\n【集数规格】: ${selectedEpisodeLength.value}\n【剧本/梗概描述】: ${userInput.value}`,
        style: activeScriptType.value
      })
    });
...
const handleApplyTemplate = (payload: { prompt: string; scriptType?: string; platform?: string; episodeLength?: string }) => {
  userInput.value = payload.prompt;
  if (payload.scriptType) activeScriptType.value = payload.scriptType;
  if (payload.platform) selectedPlatform.value = payload.platform;
  if (payload.episodeLength) selectedEpisodeLength.value = payload.episodeLength;
  if (inputCardRef.value) {
    inputCardRef.value.scrollIntoView({ behavior: 'smooth', block: 'center' });
  }
};
